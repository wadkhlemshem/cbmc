...
// BEGIN PROGRAMMING_RESEARCH_IS_CONST
  if(t->is_assign())
  {
    bool is_constant=
      to_code_assign(t->code).lhs().type().get_bool(ID_C_constant);
#ifdef DEBUG
    if(is_constant) { std::cout << "   CONST" << std::endl; }
#endif
    if(is_constant)
      return;
  }
// END PROGRAMMING_RESEARCH_IS_CONST
