...
void lock_graph_domaint::transform(const namespacet &ns, locationt from_l,
				   locationt to_l)
{
  switch (from_l->type)
  {
  case FUNCTION_CALL:
  {
	...
	lock_set_analysis->value_set_analysis[from_l].
	...
	if(which_threads->is_thread_entry(from_l->function))
        {
	  thread_categories[from_l->function] = 
	    which_threads->thread_categories[from_l->function];
        }
        else
	{
	  which_threads->used_by_thread_categories(from_l->function,
						 thread_categories);
	}
#ifdef DEBUG
	std::cout << "THREADS:";
	...
	for(which_threads_internalt::thread_categoriest::const_iterator 
	    ...
	    t_it != thread_categories.end(); ++t_it)
	{
	  if(lock) push_lock(ns,t_it->first,lock_objects.read());
	  if(unlock) pop_lock(ns,t_it->first,lock_objects.read());
	}
      } 
    } // if symbol
  } // case switch
				   ...
				   const lock_typet &lock) 
{
...
#ifdef DEBUG
  std::cout << "PUSH LOCK: "; output_lock(lock_set_analysis->ns,std::cout,lock);
  std::cout << std::endl;
#endif

  assert(!(*current_locks)[thread_category].empty());
  unsigned prev_index = (*current_locks)[thread_category].back();
  nodet node(lock);
  unsigned node_index = -1;
  bool node_exists = has_node(*lock_graph,node,node_index);
  bool edge_exists = false;
  
  if(!node_exists)
  {
    node_index = lock_graph->add_node(node);
  }
  else
  {
    edge_exists = lock_graph->has_edge(prev_index,node_index);
  }

  if(!edge_exists)
  {
    lock_graph->add_edge(prev_index,node_index);
    lock_graph->edge(prev_index,node_index) = edget(thread_category);
    assert(lock_graph->has_edge(prev_index,node_index));
    assert((*lock_graph)[prev_index].out.size()>0);
  }
  else
  {
    lock_graph->edge(prev_index,node_index).
  ...
  (*current_locks)[thread_category].push_back(node_index);
}  
...
#ifdef DEBUG
  std::cout << "POP LOCK: "; output_lock(lock_set_analysis->ns,std::cout,lock);
  std::cout << std::endl;
#endif

  std::list<typename lock_graph_domaint::grapht::node_indext>
    &locks = (*current_locks)[thread_category];

  assert(!locks.empty());
  unsigned current_index = locks.back();
  if(!lock.top && (*lock_graph)[current_index].lock != lock)
  {
#ifdef DEBUG
...
void lock_grapht::detect_deadlocks()
{
...
/*******************************************************************
 Function: lock_grapht::generate_state
...
void lock_grapht::generate_state(locationt l)
{
  state_map[l].initialize(ns, l);
  state_map[l].lock_set_analysis = &lock_set_analysis;
  state_map[l].which_threads = &which_threads;
  state_map[l].lock_graph = &lock_graph;
  state_map[l].current_locks = &current_locks;
}

/*******************************************************************
 Function: lock_grapht::initialize
...
void lock_grapht::initialize(const goto_functionst &goto_functions)
{
...
void lock_grapht::initialize(const goto_programt &goto_program)
{
  baset::initialize(goto_program);
}

/*******************************************************************
 Function: lock_grapht::output
...
void lock_grapht::output(std::ostream &out)
{
  for(lock_graph_domaint::grapht::node_indext n=0; n<lock_graph.size(); n++)
  {
...
/*******************************************************************
 Function: lock_grapht::output_deadlocks
...
void lock_grapht::output_deadlocks(std::ostream &out)
{
...
void lock_grapht::convert_node(xmlt &dest,
...
void lock_grapht::convert_edge(xmlt &dest,
			       lock_graph_domaint::grapht::node_indext n,
...
void lock_grapht::convert(xmlt &dest)
{
  ...
  for(lock_graph_domaint::grapht::node_indext n=0; n<lock_graph.size(); n++)
  {
	...
	it++)
    {
      convert_edge(dest,n,it->first);
    }
  }
}

/*******************************************************************
 Function: lock_grapht::convert_deadlocks
...
void lock_grapht::convert_deadlocks(xmlt &dest)
{
      ...
      d_it != potential_deadlocks.end(); ++d_it)
  {
    ...
    {
      assert(lock_graph.has_edge(*pp_it,*p_it));
      convert_edge(d,*pp_it,*p_it);
      convert_node(d,*pp_it);
    }
  }
}

/*******************************************************************
 Function: show_lock_graph
...
void show_lock_graph(ui_message_handlert::uit ui,
		    ...
		    lock_grapht &lock_graph)
{
  ...
  {
    xmlt xml;
    lock_graph.convert(xml);
    std::cout << xml << std::endl;
  }
  break;

  case ui_message_handlert::PLAIN:
    lock_graph.output(std::cout);
    break;
...
void show_deadlocks(ui_message_handlert::uit ui,
		    const goto_functionst &goto_functions,
		    lock_grapht &lock_graph)
{
  switch (ui)
  ...
  {
    xmlt xml;
    lock_graph.convert_deadlocks(xml);
    std::cout << xml << std::endl;
  }
  break;

  case ui_message_handlert::PLAIN:
    lock_graph.output_deadlocks(std::cout);
    break;
