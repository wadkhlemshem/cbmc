...
public:
  ...
  edget() : thread(false), k(0), inside_loop(false) {}
  edget(bool _thread, 
	irep_idt _name, 
	unsigned _k, 
	bool _inside_loop)
  ...
  struct thread_categoryt
  {
    ...
    thread_categoryt() : depth(0), inside_loop(false), k(0), nr_instructions(0)
    {}
  };
    ...
    const goto_programt::const_targett t2)
  {
    ...
    if(tc1.empty() && tc2.empty()) //both are main thread
      return false;
    if(!(tc1.size()==1 && tc2.size()==1)) //one is the main thread
      return true;

    return (tc1.begin()->first != tc2.begin()->first) || //different threads
      tc1.begin()->second.inside_loop ||                 //thread created in loop
      tc1.begin()->second.k>1;                           //multiple static thread creations
  }
