...
void lock_set_domaint::transform(const namespacet &ns, locationt from_l,
				 locationt to_l)
{
  switch (from_l->type)
  {
...
/*******************************************************************
 Function: lock_set_analysist::generate_state
...
void lock_set_analysist::generate_state(locationt l)
{
  state_map[l].initialize(ns, l);
  state_map[l].value_set_analysis = &value_set_analysis;
}
...
void lock_set_analysist::initialize(const goto_functionst &goto_functions)
{
...
void convert(const goto_functionst &goto_functions,
	     const lock_set_analysist &lock_set_analysis, xmlt &dest)
{
       ...
       f_it != goto_functions.function_map.end(); f_it++)
  {
    ...
    lock_set_analysis.convert(f_it->second.body, f_it->first, f);
  }
}
	     ...
	     const lock_set_analysist &lock_set_analysis, xmlt &dest)
{
  ...
  lock_set_analysis.convert(goto_program, "", dest.new_element("program"));
}
...
void show_lock_sets(ui_message_handlert::uit ui,
		    ...
		    const lock_set_analysist &lock_set_analysis)
{
  ...
  {
    xmlt xml;
    convert(goto_functions, lock_set_analysis, xml);
    std::cout << xml << std::endl;
  }
  break;

  case ui_message_handlert::PLAIN:
    lock_set_analysis.output(goto_functions, std::cout);
    break;
		    ...
		    const lock_set_analysist &lock_set_analysis)
{
  ...
  {
    xmlt xml;
    convert(goto_program, lock_set_analysis, xml);
    std::cout << xml << std::endl;
  }
  break;

  case ui_message_handlert::PLAIN:
    lock_set_analysis.output(goto_program, std::cout);
    break;
