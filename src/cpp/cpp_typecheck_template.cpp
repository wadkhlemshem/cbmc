  ...
  cpp_declarationt &declaration)
{
  ...
  // in the same scope.
  {
    ...
    if(!id_set.empty())
    {
      ...
      {
        error().source_location=cpp_name.source_location();
        error() << "template declaration of `" << base_name.c_str()
                << " does not match previous declaration\n";
        error() << "location of previous definition: " << previous.location
                << eom;
        throw 0;
      }
    }
  ...
  cpp_declarationt &declaration)
{
  ...
  if(declaration.is_class_template())
  {
    ...
    {
      error().source_location=declaration.source_location();
      error() << "class template not expected to have declarators" << eom;
      throw 0;
    }
  ...
  else // maybe function template, maybe class template member, maye template variable
  {
    ...
    {
      error().source_location=declaration.source_location();
      error() << "non-class template is expected to have a declarator" << eom;
      throw 0;
    }
