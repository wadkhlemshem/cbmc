...
const goto_tracet &goto_trace_storaget::insert(goto_tracet &&trace)
{
  ...
  property_to_trace.emplace(last_step.property_id, traces.size() - 1);
  return traces.back();
}
...
const goto_tracet &goto_trace_storaget::insert_all(goto_tracet &&trace)
{
  ...
  {
    property_to_trace.emplace(property_id, traces.size() - 1);
  }
  return traces.back();
}
...
const goto_tracet &goto_trace_storaget::operator[](
  const irep_idt &property_id) const
{
  const auto trace_found = property_to_trace.find(property_id);
  PRECONDITION(trace_found != property_to_trace.end());
