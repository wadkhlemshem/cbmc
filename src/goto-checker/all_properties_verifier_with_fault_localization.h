  ...
  resultt operator()() override
  {
          ...
          incremental_goto_checkert::resultt::progresst::DONE)
    {
      ...
      const goto_tracet &trace =
        traces.insert(incremental_goto_checker.build_trace());
      const irep_idt &failed_property_id = trace.get_last_step().property_id;

      fault_locations.insert(
        {failed_property_id,
         incremental_goto_checker.localize_fault(failed_property_id)});
